...
Personal portfolio project demonstrating cross-platform architecture and UX polish.
...
1. Add your token to `composeApp/src/iosMain/kotlin/com/kanav/usermanager/config/Secrets.ios.kt` (see **Token Setup** below).
...
Create the file `composeApp/src/iosMain/kotlin/com/kanav/usermanager/config/Secrets.ios.kt` with your token:
...
Offline queuing (WorkManager / BGTaskScheduler sync) was intentionally omitted — the inline error UX is clear and honest without adding sync infrastructure.
...
**Fallback strategy**: if `ListDetailPaneScaffold` fails at runtime, the fallback is a manual `WindowSizeClass` row/column layout (documented in `AdaptiveUserListLayout.kt`).
...
Create and Delete operations require active network connectivity. Offline queuing (WorkManager / BGTaskScheduler sync) was intentionally omitted to keep the scope focused. The app shows clear inline errors when offline and never silently drops user actions.

### Adaptive Layout

Adaptive behaviour (single-pane portrait, dual-pane landscape/tablet) is implemented with `ListDetailPaneScaffold` from `material3-adaptive`. A manual `WindowSizeClass` row/column fallback is documented and ready to swap in if API incompatibilities arise.
